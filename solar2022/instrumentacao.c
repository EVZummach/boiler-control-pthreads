...
static int portaUDPpNuvem;				// Porta UDP usada pela nuvem


// Sockets UDP/IP
static int socketInstrumentacao;		// Socket para a instrumentação
static int socketNuvem;					// Socket para a nuvem


// Endereço do controlador para mandar comandos como se fosse a nuvem
...
void instrumentacao_threadServidora(void)
{
	...
	/********************************
	*								*
	*	THREAD CLIENTE DA NUVEM		*
	*								*
	********************************/



/** Envia comando para o controlador e espera resposta
	Atua como se fosse uma aplicação na nuvem
	Retorna mensagem recebida do controlador
*/
char *instrumentacao_leControlador(void)
{
	struct timeval tv;
	static char resposta[1000];		// Acessado após o retorno da função
	int n;
	
	// Limpa o buffer do socket
	tv.tv_sec = 0;
	tv.tv_usec = 1000;
	setsockopt( socketNuvem, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv);
	...
	tv.tv_usec = 0;
	setsockopt( socketNuvem, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv);
	n = recebeMensagem( socketNuvem, resposta, 1000);
